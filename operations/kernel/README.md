...
2. **Single source of truth**. The delivery manifest (`operations/delivery/manifests/delivery.manifest.json`) describes the topology; the kernel validates behavior against it.
...
- `operations/kernel/contracts/health.contract.ts` — `/api/health` contract and validator.
- `scripts/ops/verify-deployment.ts` — deployment verification (DC-6 + DC-6.5).
- `scripts/ops/assert-health-contract.ts` — one-shot health contract assertion for CI.

...
| --- | --- | --- | --- | --- | --- | --- |
| `E2E_BASE_URL` | Base URL for local Playwright end-to-end tests. | development | local-env | No | No | — |
| `TRACE_ID` | Optional trace identifier for SCGS replay validation. | development | local-env | No | No | — |
...
| `ALGOLIA_SEARCH_INDEX_NAME` | Primary Algolia search index name. | server-runtime | fly-secrets | No | No | `parts` |
| `ALGOLIA_INDEX_PRICE_ASC` | Algolia index used for price ascending sort. | server-runtime | fly-secrets | No | No | — |
| `ALGOLIA_INDEX_PRICE_DESC` | Algolia index used for price descending sort. | server-runtime | fly-secrets | No | No | — |
| `ALGOLIA_INDEX_NEWEST` | Algolia index used for newest-first sort. | server-runtime | fly-secrets | No | No | — |

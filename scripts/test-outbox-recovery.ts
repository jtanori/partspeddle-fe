...
async function runOutboxRecoveryTest() {
  ...
  const reportLines: string[] = ['# Stage 5: Outbox Recovery Validation Report'];
  ...
  const finalVerdict = cleanShutdownResult.passed && crashTestResult.passed && retryStormResult.passed ? '✅ PASS' : '❌ FAIL';
...
async function runCleanShutdownTest(): Promise<{ passed: boolean, report: string[] }> {
    ...
    try {
        ...
        const { error: insertError } = await supabaseAdmin.from('parts').insert(newParts);
        if (insertError) throw new Error(`Could not generate test events: ${insertError.message}`);
        reportLines.push('- ✅ 100 new parts created, generating 100 outbox events.');

        await new Promise(resolve => setTimeout(resolve, 2500));
        
        const { count: pendingCount, error: countError } = await supabaseAdmin.from('search_outbox').select('*', { count: 'exact', head: true }).in('aggregate_id', testPartIds).eq('processed', false);

        if (countError || pendingCount !== 100) {
            throw new Error(`Expected 100 pending events, but found ${pendingCount}. Error: ${countError?.message}`);
        }
        reportLines.push("- ✅ Verified: **100** pending events found in `search_outbox`.");
        ...
        const { count: finalPending } = await supabaseAdmin.from('search_outbox').select('*', { count: 'exact', head: true }).in('aggregate_id', testPartIds).eq('processed', false);
        const { count: finalProcessed } = await supabaseAdmin.from('search_outbox').select('*', { count: 'exact', head: true }).in('aggregate_id', testPartIds).eq('processed', true);
        const { results } = await algoliaClient.getObjects(testPartIds.map(objectID => ({ indexName: SEARCH_INDEX_NAME, objectID, attributesToRetrieve: ['objectID'] })));
        const indexedCount = results.filter(r => r !== null).length;

        passed = finalPending === 0 && finalProcessed === 100 && indexedCount === 100;
        ...
        reportLines.push('', `**CRITICAL ERROR:** ${error.message}`, '', '### Test 1 Verdict: ❌ FAIL');
    } finally {
        await cleanup(testPartIds);
...
async function runMidProcessingCrashTest(): Promise<{ passed: boolean, report: string[] }> {
    ...
    try {
        ...
        const { error: insertError } = await supabaseAdmin.from('parts').insert(newParts);
        if (insertError) throw new Error(`Could not generate test events for crash test: ${insertError.message}`);
        ...
        passed = finalPending === 0 && finalProcessed === 200 && indexedCount === 200;
        ...
        reportLines.push('', `**CRITICAL ERROR:** ${error.message}`, '', '### Test 2 Verdict: ❌ FAIL');
    } finally {
        await cleanup(testPartIds);
...
async function runRetryStormTest(): Promise<{ passed: boolean, report: string[] }> {
    ...
    try {
        ...
        const { error: insertError } = await supabaseAdmin.from('parts').insert(newParts);
        if (insertError) throw new Error(`Could not generate test events for retry test: ${insertError.message}`);
        ...
        const invalidEnv = { ...process.env, VITE_ALGOLIA_ADMIN_API_KEY: 'invalid_key' };
        try {
            await runWorker(invalidEnv);
        } catch (error) {
            // Expected to fail
        }
        reportLines.push('- ✅ Worker process executed with invalid credentials and failed as expected.');

        const { data: events, error: eventsError } = await supabaseAdmin.from('search_outbox').select('retry_count, processed').in('aggregate_id', testPartIds);
        if (eventsError) throw new Error(`Could not fetch events from outbox: ${eventsError.message}`);
        ...
        if (retriedEvents < 50 || stillUnprocessed < 50) {
            throw new Error(`Expected all 50 events to be retried and remain unprocessed, but found ${retriedEvents} retried and ${stillUnprocessed} unprocessed.`);
        }
        reportLines.push(`- ✅ Verified: **${retriedEvents}** events correctly marked for retry.`);

        reportLines.push('', '## Phase 3: Recover and Reprocess');
        await runWorker();
        reportLines.push('- ✅ Worker process restarted with correct credentials and finished processing.');
        ...
        reportLines.push('', `**CRITICAL ERROR:** ${error.message}`, '', '### Test 3 Verdict: ❌ FAIL');
    } finally {
        await cleanup(testPartIds);
...
async function cleanup(partIds: string[]) {
  ...
  await supabaseAdmin.from('search_outbox').delete().in('aggregate_id', partIds);
}

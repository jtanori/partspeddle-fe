...
async function reconcileSearchIndex() {
  ...
  try {
    ...
    // 3. Compare
    const missingInAlgolia = [...dbPartIds].filter(id => !algoliaPartIds.has(id));
    const extraInAlgolia = [...algoliaPartIds].filter(id => !dbPartIds.has(id));
    ...
    if (missingInAlgolia.length > 0) {
      console.warn('⚠️ Found parts missing in Algolia. Run reindex for these IDs.');
    }

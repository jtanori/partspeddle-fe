...
async function seedListingsSecure() {
  ...
  if (sellerError || !sellers || sellers.length === 0) {
    console.error(
      '❌ Error Relacional: No se encontraron perfiles en "seller_profiles". Ejecuta primero el seed de usuarios.',
    );
    process.exit(1);
  }
  ...
  for (let i = 0; i < mockListings.length; i++) {
    ...
    if (!VALID_SYSTEMS.includes(item.system)) {
      console.error(
        `❌ Error de validación local: El sistema '${item.system}' no pertenece a la taxonomía permitida.`,
      );
      process.exit(1);
    }
  ...
  console.log(
    `📤 Insertando ${finalListings.length} listings estructurados de forma segura...`,
  );
  ...
  if (insertError) {
    console.error(
      "❌ Error de Postgres al insertar el inventario:",
      insertError.message,
    );
    process.exit(1);
  }
  ...
  console.log(
    `✨ Completado: Se han sembrado los ${data.length} listings. El compresor Sanden del Honda CR-V 2017 quedó activo.`,
  );
}

...
async function runDriftValidationTest() {
    ...
    const reportLines: string[] = ['# Stage 5: Search Drift Validation Report', ''];
    ...
    const finalVerdict = scenarioAResult.passed && scenarioBResult.passed && scenarioCResult.passed && scenarioDResult.passed && largeDatasetResult.passed ? '✅ PASS' : '❌ FAIL';
...
'];
    ...
    try {
        ...
        passed = auditResult.missingFromIndex.length === 0 && auditResult.staleInIndex.length === 0 && auditResult.attributeMismatches.length === 0;
...
'];
    ...
    try {
        reportLines.push('## Phase 1: Setup and Event Generation');
        await cleanup(testPartIds);
        const newParts: Part[] = testPartIds.map(id => ({ id, title: `DRIFT-TEST-B-OLD-${faker.commerce.productName()}`, status: 'available' }));
        ...
        const updates = partsToUpdate.map(id => ({ id, title: `DRIFT-TEST-B-NEW-${faker.commerce.productName()}` }));
        const { error: updateError } = await supabaseAdmin.from('parts').upsert(updates);
        if (updateError) throw new Error(`Could not update parts: ${updateError.message}`);
...
'];
    ...
    try {
        reportLines.push('## Phase 1: Setup and Event Generation');
        await cleanup(testPartIds);
        const newParts: Part[] = testPartIds.map(id => ({ id, title: `DRIFT-TEST-C-${faker.commerce.productName()}`, status: 'available' }));
        await supabaseAdmin.from('parts').insert(newParts);
        await runWorker();
        reportLines.push('- ✅ 100 new parts created and synced.');
        ...
        const { error: deleteError } = await supabaseAdmin.from('parts').delete().in('id', partsToDelete);
        if (deleteError) throw new Error(`Could not delete parts: ${deleteError.message}`);
...
'];
    ...
    try {
        reportLines.push('## Phase 1: Setup and Event Generation');
        await cleanup(testPartIds);
        const newParts: Part[] = testPartIds.map(id => ({ id, title: `DRIFT-TEST-D-${faker.commerce.productName()}`, status: 'available' }));
        await supabaseAdmin.from('parts').insert(newParts);
        await runWorker();
        reportLines.push('- ✅ 100 new parts created and synced.');
        ...
        await algoliaClient.deleteObjects({ indexName: SEARCH_INDEX_NAME, objectIDs: partsToDelete });
        reportLines.push('- ✅ 20 records manually deleted from Algolia.');

        reportLines.push('', '## Phase 3: Audit and Verification');
        const auditResult = await audit();
        
        passed = auditResult.missingFromIndex === 20 && auditResult.staleInIndex === 0;
        ...
        reportLines.push('', `**CRITICAL ERROR:** ${error.message}`, '', '### Test 4 Verdict: ❌ FAIL');
    } finally {
        await cleanup(testPartIds);
...
async function runLargeDatasetValidation(): Promise<{ passed: boolean, report: string[] }> {
    const reportLines: string[] = ['
...
'];
    ...
    try {
        reportLines.push('## Phase 1: Setup and Event Generation');
        await cleanup(testPartIds);
        const newParts: Part[] = testPartIds.map(id => ({ id, title: `DRIFT-TEST-LARGE-${faker.commerce.productName()}`, status: 'available' }));
        await supabaseAdmin.from('parts').insert(newParts);
        await runWorker();
        ...
        passed = auditResult.missingFromIndex === 0 && auditResult.staleInIndex === 0;
        ...
        reportLines.push('', `**CRITICAL ERROR:** ${error.message}`, '', '### Test 5 Verdict: ❌ FAIL');
    } finally {
        await cleanup(testPartIds);
...
async function cleanup(partIds: string[]) {
    await algoliaClient.deleteObjects({ indexName: SEARCH_INDEX_NAME, objectIDs: partIds });

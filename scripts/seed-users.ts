...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  console.error('❌ Error: Variables de entorno de Supabase faltantes en el entorno local.');
  process.exit(1);
}
...
const MOCK_USERS = [
  // VENDEDORES (Sellers) - Usando Alias de Gmail
  {
    ...
    role: 'seller',
    meta: { full_name: 'Jaime Tanori (Seller 1)', company: 'Desert Auto Spares' }
  },
  {
    email: 'jaime.tanori+seller2@gmail.com',
    password: 'TestPassword2026!',
    role: 'seller',
    meta: { full_name: 'Jaime Tanori (Seller 2)', company: 'Baja Pacific Parts' }
  },
  {
    email: 'jaime.tanori+seller3@gmail.com',
    password: 'TestPassword2026!',
    role: 'seller',
    meta: { full_name: 'Jaime Tanori (Seller 3)', company: 'Borderline Dismantlers' }
  },
  // COMPRADORES (Buyers) - Usando Alias de Gmail
  {
    ...
    password: 'TestPassword2026!',
    role: 'buyer',
    meta: { full_name: 'Jaime Tanori (Buyer 1)', company: 'Taller Mecánico Ruiz' }
  },
  {
  ...
  },
  {
    ...
    password: 'TestPassword2026!',
    role: 'buyer',
    meta: { full_name: 'Jaime Tanori (Buyer 3)', company: 'Arce Reconstructores' }
  }
];

async function seedUsers() {
  console.log('🌱 Iniciando la siembra de usuarios con alias personalizados...');
  ...
  for (const mockUser of MOCK_USERS) {
    ...
    if (authError) {
      if (authError.message.includes('already exists') || (authError as any).status === 422) {
        console.log(`⚠️ El alias ${mockUser.email} ya está registrado. Saltando al siguiente...`);
        continue;
      }
      console.error(`❌ Error al registrar en auth para ${mockUser.email}:`, authError.message);
      continue;
    }
    ...
    if (userSyncError) {
      console.error(`❌ Error al sincronizar public.users para ${mockUser.email}:`, userSyncError.message);
    }

    // 2. Hidratación de tablas públicas de perfiles según el rol
    if (mockUser.role === 'seller') {
      console.log(`📤 Creando registro en 'seller_profiles' para el negocio: ${mockUser.meta.company}`);
      ...
      if (sellerError) {
        console.error(`❌ Error en tabla 'seller_profiles' para ID ${userId}:`, sellerError.message);
      }
    }
  }
...
seedUsers().catch((err) => {
  console.error('💥 Fallo crítico en el hilo de ejecución del script de usuarios:', err);
});

...
| DC-6   | CI verification jobs + recovery runbook          | Implemented   | Runtime env check in `/api/health`; `scripts/ops/verify-deployment.ts`; CI smoke-staging job runs `pnpm ci:verify:staging` before smoke tests; `docs/operations/recovery-runbook.md` created. Pending CI validation. |

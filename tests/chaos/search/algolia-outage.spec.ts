...
vi.mock("@/backend/modules/search/infrastructure/algolia-client", async () => {
  const actual = await vi.importActual(
    "@/backend/modules/search/infrastructure/algolia-client",
  );
  return {
    ...(actual as any),
    algoliaClient: {
...
describe("Search Chaos Suite: Algolia Outage", () => {
  it("handles Algolia outage gracefully", async () => {
    ...
    (algoliaClient.search as any).mockRejectedValue(
      new Error("Algolia is unavailable"),
    );
    ...
    expect(data.warning).toBe(
      "Search currently unavailable: Algolia is unavailable",
    );
  });
});

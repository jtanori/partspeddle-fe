...
describe('P3.1 dead code and stale comment cleanup', () => {
  it('removes legacy Express search contract handlers', () => {
    expect(exists('src/backend/modules/search/contracts')).toBe(false);
    expect(exists('src/backend/modules/search/contracts/search-api-handler.ts')).toBe(
      false,
    );
    expect(exists('src/backend/modules/search/contracts/search-suggestions-handler.ts')).toBe(
      false,
    );
  });

  it('removes orphaned legacy dashboard and dev-only components', () => {
    expect(exists('src/components/SellerDashboard.tsx')).toBe(false);
    expect(exists('src/components/ComponentLibrary.tsx')).toBe(false);
    expect(exists('src/components/navbar/LiveSearchDropdown.tsx')).toBe(false);
  });
  ...
  it('replaces facet parity TODO with an explicit deferred note', () => {
    const parity = read('src/projection/search-parity.ts');

...
export class AlgoliaSearchRepository implements SearchRepository {
  async search(
    ...
    const result = await algoliaClient.search<SearchDocument>({
      requests: [
        {
          indexName,
          query,
          ...(algoliaFilters.trim().length > 0 ? { filters: algoliaFilters } : {}),
          page,
          hitsPerPage,
          facets: [
            'category',
    ...
    const response = result.results[0] as SearchResponse<SearchDocument> | undefined;
    ...
    if ('error' in response && response.error) {
      throw new Error(`Algolia Search Error: ${String(response.error)}`);
    }
  ...
  buildAlgoliaFilters(filters: SearchFilters): string {
    ...
    const addFilter = (field: string, values: string[] | undefined) => {
      if (!values || !Array.isArray(values)) return;
      const validValues = values.filter((v) => typeof v === 'string' && v.trim() !== '');

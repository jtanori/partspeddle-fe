...
- `src/middleware.ts` generates a 16-byte cryptographically-secure nonce on every request using Web Crypto.
- The middleware sets `Content-Security-Policy` via `buildContentSecurityPolicy(nonce)` from `src/lib/security-headers.ts`.
...
- `src/middleware.ts` — nonce generation and request-scoped CSP header.

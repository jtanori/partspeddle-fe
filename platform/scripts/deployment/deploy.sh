...
if [[ -z "$ENVIRONMENT" ]]; then
  ...
  echo "Usage: bash platform/scripts/deployment/deploy.sh {staging|production}"
  exit 1
fi

case "$ENVIRONMENT" in
  staging)
    CONFIG="platform/deployment/fly/fly.stage.toml"
    EXPECTED_BRANCH="develop"
    ENV_FILE=".env.staging"
    ;;
  production)
    CONFIG="platform/deployment/fly/fly.prod.toml"
...
flyctl deploy --config "$CONFIG" --dockerfile "platform/docker/Dockerfile" --build-arg "ENV_FILE=$ENV_FILE"

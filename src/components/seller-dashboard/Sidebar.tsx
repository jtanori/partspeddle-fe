...
export const SellerSidebar: React.FC<SellerSidebarProps> = ({
  ...
  return (
    <aside
      className={cn(
        'w-[260px] h-screen flex flex-col justify-between border-r border-border-default p-4 shrink-0 bg-shell-sidebar font-mono text-xs select-none shadow-panel relative z-20',
        className,
      )}
    >
      <div className="space-y-6">
        <div className="px-2 py-4 border-b border-dashed border-zinc-800 pb-6 flex items-start justify-between gap-2">
          <img src={logoSolidImg.src} alt="PartsPeddle" className="w-full h-auto object-contain" />
          {showCloseButton && onClose && (
            <button
              onClick={onClose}
              className="p-2 -mr-1 -mt-1 text-text-muted hover:text-accent-amber transition-colors shrink-0"
        ...
        <SidebarLink to="/dashboard/snap" label="CREATE NEW LISTING" icon={Plus} onNavigate={onNavigate} />
              ...
              <div key={tab.href}>
                <div className="flex items-center">
                  <div className="flex-1">
                    <SidebarLink to={tab.href} label={tab.label} icon={Icon} onNavigate={onNavigate} />
                  </div>
                  {hasChildren && (
                    <button 
                      onClick={(e) => { e.preventDefault(); toggleExpand(tab.href); }}
                      className="p-2 text-text-muted hover:text-accent-amber transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center"
                ...
                {hasChildren && isExpanded && (
                  <div className="pl-10 pr-2 space-y-1 mt-1 border-l border-border-subtle ml-5">
                    {tab.children.map(child => (
                      <SidebarLink key={child.href} to={child.href} label={child.label} icon={GitBranch} onNavigate={onNavigate} />
                    ))}
                  </div>
                )}
              </div>
      ...
      <div className="border-t border-dashed border-zinc-800 pt-6 space-y-2">
        <Link
          href="/search"
          onClick={onNavigate}
          className="flex items-center gap-3 px-3 py-2.5 rounded-sm text-text-muted hover:text-accent-amber transition-all min-h-[44px]"
        >
        ...
        <button
          ...
          className="w-full flex items-center gap-3 px-3 py-2.5 text-danger/80 hover:text-danger hover:bg-danger/10 transition-all text-left font-mono rounded-sm min-h-[44px]"
        >

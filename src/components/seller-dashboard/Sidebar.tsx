...
export const SellerSidebar: React.FC = () => {
  const { activeSellerTab, setActiveSellerTab, logout } = useAppStore();
  ...
  return (
    <aside className="w-64 h-full flex flex-col justify-between border-r border-amber-500/10 p-4 shrink-0 bg-neutral-950 font-mono text-xs select-none">
      <div className="space-y-6">
        ...
        <a href="/" className="flex items-center gap-3 px-2 py-1 group border-b border-dashed border-zinc-800 pb-4">
        ...
        <nav className="space-y-1">
          {[
            { id: 'listings', label: 'OVERVIEW', icon: LayoutDashboard },
            { id: 'inventory', label: 'INVENTORY', icon: Package },
            { id: 'snap', label: 'CREATE LISTING', icon: PlusSquare },
            { id: 'orders', label: 'ORDERS', icon: ShoppingBag },
            { id: 'settings', label: 'YARD SETTINGS', icon: Settings },
          ].map((tab) => {
            const Icon = tab.icon;
            const isActive = activeSellerTab === tab.id || (tab.id === 'snap' && activeSellerTab === 'create');
            return (
              <button
                key={tab.id}
                onClick={() => setActiveSellerTab(tab.id === 'snap' ? 'snap' : tab.id as any)}
      ...
      <div className="border-t border-dashed border-zinc-800 pt-4 space-y-1">
        ...
        <button
          onClick={logout}
          className="w-full flex items-center gap-3 px-3 py-2.5 text-red-400/80 hover:text-red-400 hover:bg-red-950/20 transition-all"
        >

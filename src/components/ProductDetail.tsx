...
export default function ProductDetail({
  ...
  const handlePrevPhoto = () =>
    setActiveImageIdx(
      (prev) =>
        (prev - 1 + (detailPartImages[part?.id || "1100428"]?.length || 4)) %
        (detailPartImages[part?.id || "1100428"]?.length || 4),
    );
  ...
  const triggerSendOffer = async (e: React.FormEvent) => {
    ...
    setTimeout(() => setNegotiationState("replied"), 1500);
  };
  const getConditionColor = (cond: string) => {
    const c = cond.toLowerCase();
    if (c.includes("new") || c.includes("original"))
  ...
  const inlinePartImages =
    detailPartImages[part?.id || "1100428"] || detailPartImages["1100428"];
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          >
            ...
            <button
              ...
              className="w-full bg-[#B87333] hover:bg-[#8B6239] text-white font-display font-bold uppercase tracking-wider py-3.5 px-4 rounded-sm shadow-md"
            >
              ADD TO CART
            </button>
            <button
              onClick={() => setIsOfferModalOpen(true)}
              className="w-full border-2 border-zinc-800 text-zinc-800 hover:bg-zinc-50 font-display font-bold uppercase tracking-wider py-3 px-4 rounded-sm transition-all text-center"
            >
              MAKE OFFER
            </button>
          </div>

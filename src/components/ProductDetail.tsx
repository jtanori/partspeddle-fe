...
export default function ProductDetail({
  ...
  if (!part) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#F5F0EB] text-zinc-500 space-y-4">
        ...
        <button
          onClick={onBack}
          className="text-[#B87333] hover:underline uppercase text-sm font-bold"
        >
          Back to Marketplace
        </button>
      </div>
  ...
  if (!seller) {
    return null; // Or show loading state if seller is required
  }

  return (
    <div
      className="max-w-7xl mx-auto px-4 py-8 font-sans space-y-12"
      id="tour-part-view"
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-7 space-y-8">
          ...
          <div className="relative rounded overflow-hidden aspect-video border border-zinc-200 bg-zinc-950 shadow flex items-center justify-center group">
            ...
            >
              <Heart
                className={`w-4 h-4 ${favorites.includes(part.id) ? "fill-red-500 text-red-500" : ""}`}
              />
            </button>

            {/* Condition float */}
            <span
              className={`absolute bottom-4 left-4 text-xs font-display font-medium uppercase tracking-wider py-1 px-3.5 rounded-sm shadow-md ${getConditionColor(part.condition)}`}
            >
              {part.condition}
            </span>
          </div>
          ...
          <div className="p-5 bg-zinc-900 border border-zinc-700/80 rounded shadow-md text-white grid grid-cols-1 md:grid-cols-2 gap-6 relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-xs font-mono text-zinc-400">
              <div>
                ...
                <span className="text-zinc-200 text-[11px] truncate uppercase block">
                  {part.vinRemovedFrom || "N/A"}
                </span>
              </div>
              <div>
                <span className="text-zinc-500 block">Stock Code:</span>
                <span className="text-zinc-200 text-[11px] uppercase block">
                  {part.stockNumber || "N/A"}
                </span>
              </div>
              <div>
                <span className="text-zinc-500 block">Harvest Date:</span>
                <span className="text-zinc-200 text-[11px] block">
                  {part.dateRemoved || "N/A"}
                </span>
              </div>
              <div>
                <span className="text-zinc-500 block">Inspected By:</span>
                <span className="text-zinc-350 text-[11px] block">
                  Shop Certified (Bill)
                </span>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-zinc-150 font-sans">
                  {part.compatibility.map((fit, idx) => (
                    ...
                    >
                      ...
                      <td className="py-2.5 px-3 text-[#B87333] font-semibold">
                        {fit.engine}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div
            className="bg-white border border-zinc-250 rounded p-6 shadow-xl space-y-5"
            id="tour-cta"
          >
          ...
          <div className="bg-white border border-zinc-250 rounded p-6 shadow-sm space-y-4">
            ...
            <div className="space-y-2 text-xs pt-2 border-t border-zinc-100">
              <div className="flex items-center justify-between text-zinc-650">
                ...
                <span className="font-mono font-bold text-[#7A8B6F]">
                  {seller.feedbackPercentage}% Positive
                </span>
              </div>
              <div className="flex items-center justify-between text-zinc-650">
                <span>Shipping Dispatch:</span>
                <span className="font-sans font-medium text-zinc-850">
                  Dispatches in {seller.shipsWithin}
                </span>
              </div>
              <div className="flex items-center justify-between text-zinc-650">
                <span>Warrant Period:</span>
                <span className="font-sans text-[#B87333] font-semibold">
                  {seller.returnPolicy} guarantee
                </span>
              </div>
            </div>

  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <Logo href="/" className="w-10 h-12" id="id-mobile-nav-logo" />
        <button
          ...
          className={`transition-colors cursor-pointer py-1.5 px-1 flex items-center justify-center text-xs font-bold uppercase tracking-widest ${
            pathname === '/search' ? 'text-rust-copper' : 'text-warm-gray hover:text-rust-copper'
          }`}
        >
          Browse
        </button>
      ...
      <div className="flex items-center gap-2 font-sans">
        ...
        <BottomSheet isOpen={isUserMenuDrawerOpen} onClose={() => setIsUserMenuDrawerOpen(false)}>
          <UserMenuContent
            user={user}
            userRole={userRole}
            profile={profile}

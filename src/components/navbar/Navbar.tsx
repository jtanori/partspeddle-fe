...
export default function Navbar({
  ...
  return (
    <>
      <header 
        className="bg-white text-zinc-900 sticky top-0 z-40 border-b border-zinc-200 h-16 shadow-sm" 
        id="id-navbar-header"
      >
        <div className="hidden md:flex max-w-7xl mx-auto px-4 h-full items-center justify-between gap-5">
            ...
            <nav className="flex items-center gap-6 font-heading">
                <button 
                    ...
                    className={`transition-colors cursor-pointer border-b-2 py-1.5 px-1 min-h-[44px] flex items-center justify-center text-sm font-medium ${
                        currentView === 'listing' ? 'text-amber-600 border-amber-600' : 'text-zinc-600 hover:text-amber-600 border-transparent'
                    }`}
                >
                    BROWSE PARTS
                </button>
          ...
          <button
            ...
            className="bg-zinc-50 border border-zinc-200 rounded-sm w-10 h-10 text-zinc-700 transition-all duration-150 hover:border-amber-500 hover:text-amber-600 flex items-center justify-center cursor-pointer min-[860px]:hidden portrait:flex shadow-sm"
          ...
          {userRole === 'buyer' && (
            <button 
              ...
              className="bg-zinc-50 border border-zinc-200 rounded-sm w-10 h-10 text-zinc-700 transition-all duration-150 hover:border-amber-500 hover:text-amber-600 flex items-center justify-center cursor-pointer relative shadow-sm"
              id="nav-cart-trigger"
              title="Parts cart manager"
            >
              <ShoppingCart className="w-5 h-5" />
              {cartCount > 0 && (
                <span className="absolute -top-1 -right-1 w-4 h-4 bg-amber-600 text-white text-[10px] font-mono font-black rounded-full ring-2 ring-white flex items-center justify-center">
                  {cartCount}
                </span>
              )}
            </button>
          ...
          <button 
            onClick={cta.onClick}
            className="h-[40px] flex items-center justify-center bg-zinc-900 text-white rounded-sm px-5 text-[11px] font-black font-heading tracking-[0.15em] active:translate-y-[0.5px] cursor-pointer select-none whitespace-nowrap shadow-sm transition-all duration-150 relative overflow-hidden"
          >
